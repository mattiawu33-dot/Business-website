...
populated with a small representative catalog and clearly-labeled placeholder
photography, ready to backfill in Phase 2.
...
- `src/data/products.ts` — the catalog (16 items across Men/Women, tagged
  `isNew` / `isBestSeller`). `src/data/categories.ts` drives nav + tiles.
- `scripts/generate-placeholder-images.mjs` — generates the consistent-style
  placeholder SVG art in `public/images`. Re-run after adding products so new
  SKUs get placeholder art automatically: `node scripts/generate-placeholder-images.mjs`
...
1. Drop real (AI-enhanced) photography into `public/images/products/<slug>/`,
   `public/images/tiles/`, `public/images/hero.svg`-equivalent, etc.,
   replacing the placeholder files 1:1 by path.
